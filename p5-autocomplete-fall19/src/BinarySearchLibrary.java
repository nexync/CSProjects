...
public class BinarySearchLibrary {
	...
	public static <T>
		...
		while (low+1 != high) {
			int mid = (low + high)/2;
			if(comp.compare(target,list.get(mid)) >= 1)	low = mid;
			else	high = mid;			
		}
		if(comp.compare(target, list.get(high))==0)	return high;
		return -1;
	}
	...
	int lastIndex(List<T> list, 
		...
		while (high-1 != low) {
			int mid = (low + high)/2;
			if(comp.compare(target,list.get(mid)) <= -1) high = mid;
			else low = mid;
		}
		if(comp.compare(target, list.get(low))==0)	return low;
		return -1;
	}
