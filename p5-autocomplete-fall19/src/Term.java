...
public class Term implements Comparable<Term> {
	...
	public Term(String word, double weight) {
		...
		if (word == null)	throw new NullPointerException("invalid word "+word);
		myWord = word;
		myWeight = weight;
	}
	...
	public static class PrefixOrder implements Comparator<Term> {
		...
		public int compare(Term v, Term w) {
			...
			int index = Math.min(Math.min(v.myWord.length(), w.myWord.length()), myPrefixSize);
			for(int i = 0; i<index; i++) {
				if(v.myWord.charAt(i) > w.myWord.charAt(i))	return 1;
				else if(v.myWord.charAt(i) < w.myWord.charAt(i))	return -1;
				else	continue;	
			}
			if(index < myPrefixSize) {
				if(v.myWord.length()>w.myWord.length())	return 1;
				else if(v.myWord.length()<w.myWord.length())	return -1;
				else	return 0;
			}
			return 0;
		}
